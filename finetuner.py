...
def test(net,test_loader):
    ...
    print(f"Testing on {test_loader.name}, imgs = {len(test_loader)*test_loader.batch_size}")
...
def validate(net, val_loader ):
    ...
    net.eval()
    loss_fn = nn.CrossEntropyLoss()
    print(f"Validating on {val_loader.name}, imgs = {len(val_loader)*val_loader.batch_size}")
    with torch.inference_mode():
        correct = total = 0
        loss_sum = 0.0
...
def train(net, train_loader, epochs, learning_rate, momentum, weight_decay):
    ...
    # EVALUATE ON FINEGRAINED 
    print("Evaluating the model on the Finegrained Contexts")
    loaders.evaluate_finegrained(net,plots_dir + "finegrained_histogram_pre_FT")
    ...
    print(f"Training {net.name} on {train_loader.name} dataset, for Epochs: {epochs}, with  bs={bs}, eff_bs={None if bs is None else bs*grad_accum}, "
          f"imgs={None if bs is None else bs*len(train_loader)}, res = 224x224x3 lr={learning_rate}, mom={momentum}, wd={weight_decay}")
...
def train_ODL(net,epochs,learning_rate,train_loader = loaders.mini_WV_train_ld):
    ...
    for epoch in range(epochs):
        for x, y,*_ in train_loader:
            ...
            # forward entirely in FP16
            logits = net(x)

            loss = loss_fn(logits, y).half()
