...
class FastWVCacheDatasetV2(Dataset):
    ...
    def __init__(self, cache_dir, cond=None, mode="AND", max_shards_in_mem=2, name="Wake Vision"):
        self.name = name
        print(f"Initializing Dataset {self.name}, this will take a few minutes")
        self.files = sorted(glob.glob(os.path.join(cache_dir, "shard-*.pt")))
        assert self.files, f"no shards in {cache_dir}"
...
# Usage
WakeVision_train = FastWVCacheDatasetV2("./datasets/wv_train_cache_224",name = "WakeVision_train_quality")
WakeVision_val   = FastWVCacheDataset("./datasets/wv_val_cache_224",name = "WakeVision_val" )
...
def evaluate_finegrained(model,save_path, cache_dir = "./datasets/wv_val_cache_224", batch_size=16, device="cpu"):
    ...
    dl = DataLoader(ds, batch_size=batch_size, shuffle=False,
                    num_workers=2, pin_memory=pin_memory, collate_fn=collate_xy)
    assert len(ds.fg_keys) > 0, "fg_keys empty"
    K = len(ds.fg_keys)
    ...
    with torch.inference_mode():
        for x, y, fg, _ in dl:
            ...
            fg_bool = fg.bool()                          # [B,K] on CPU
            # vectorized counts
            tot += fg_bool.sum(0).to(torch.long)         # per-tag support
            tp  += (correct.unsqueeze(1) & fg_bool).sum(0).to(torch.long)
    ...
    plt.ylabel("Accuracy")
    plt.title(f"Wake Vision validation fine-grained accuracy (overall={acc_all:.3f})")
    plt.tight_layout()
